...
const getAuthHeaders = () => ({
  ...
  const fetchExistingData = async () => {
    try {
      const response = await axios.get(
        `/api/api/demandnoticeCB/case/${caseId}`,
        {
      ...
      if (response.data && response.data.status === 'SUCCESS' && response.data.data) {
        ...
        setFormData({
          noticeDeadline: noticeData.noticeDeadline || "",
          noticeSentDate: noticeData.noticeSentDate || "",
          noticeType: noticeData.noticeType || "Cheque Bounce",
          comment: noticeData.comment || "",
          dispositions: noticeData.dispositions || [],
        });
      }
    } catch (error) {
      console.error("Error fetching demand notice:", error.response || error);
  ...
  const handleSaveAndNext = async () => {
    ...
    try {
      ...
      console.error("Error saving demand notice:", error.response || error);
      console.error("Error details:", {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data
        ...
        <div className="chequeBounce-demandNotice-topcontent">
          ...
          <div className="chequeBounce-demandNotice-topcontent-rightside">
            <button
              ...
              disabled={
                loading || !formData.noticeDeadline || !formData.noticeSentDate
              }
      ...
      <div className="chequeBounce-demandNotice-Bottom-btn">
        ...
        <SaveButton
          ...
          disabled={
            loading ||
            (!hasExistingData &&
              (!formData.noticeDeadline || !formData.noticeSentDate))
          }
        />
      </div>
